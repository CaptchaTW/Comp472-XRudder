    ...
    while game1.get_move_counter() != 30 or player1.get_pieces_counter() != 15 or player2.get_pieces_counter() != 15:
        ...
        while True:
            ...
            if game1.get_turn_counter()%2 == game1.get_AI_turn():
                ...
                print(column_holder,row_holder,move_column_holder,move_row_holder,score,movetype)
                if movetype =="Put":
                    array_of_players[turn_counter].put_piece(game1,column_holder,row_holder)
                    print("Put token " +column_holder+str(row_holder))
                    if game1.check_winning_conditions(array_of_players[turn_counter],column_holder,row_holder):
                    ...
                    break
                if movetype =="Move":
                    array_of_players[turn_counter].move_piece(game1, column_holder, row_holder, move_column_holder, move_row_holder)
                    print("Moved token " +column_holder+str(row_holder)+" to"+ move_column_holder+str(move_row_holder))
                    if game1.check_winning_conditions(array_of_players[turn_counter], move_column_holder, move_row_holder):
                        print(game1.print_board())
                        print("Player " + str(turn_counter + 1) + " is the Winner")
                        input("Enter any key to end program")
                        os._exit(0)
